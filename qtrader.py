...
class QTrader(object):
    def __init__(self, path='', eta=1, transaction_cost=0.005, position=10000):
        self.eta = eta
        self.transaction_cost = transaction_cost
        self.stock_data = pd.merge(pd.read_csv(path+'/tbill.csv', index_col='Date'), pd.read_csv(path+'/^GSPC.csv', index_col='Date'), right_index=True, left_index=True).sort_index()
